...
function setup() {
  ...
  restart = createSprite(width/2,height/4+40);
...
function draw() {
  //trex.debug = true;
  background(255);
  text("Score: "+ score, windowWidth/4 + camera.position.x ,50);
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    var cloud = createSprite(camera.position.x + windowWidth/4 ,height-20,40,10);
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(camera.position.x + windowWidth/2,height-50,10,40);
    //obstacle.debug = true;
